...
struct ReceivedPacket {
     ...
     */
    ReceivedPacket(const SFML::IpAddress& ip, std::uint16_t port, const std::string& packet_data)
        : sender(ip, port)
     ...
     * @param packet_data The data of the packet
     */
    ReceivedPacket(const UDPClient& sender, const std::string& packet_data)
        : sender(sender)
        , packet_data(packet_data)
    {
    }
