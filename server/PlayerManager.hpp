...
namespace RType {
    ...
    public:
         ...
         * @param udp_handler The class that handles sending packet to other UDPClients
         */
        void sendGamestartToAllPlayers(std::shared_ptr<Network::UDPHandler> &udp_handler, std::shared_ptr<RType::Network::PackageManager> &package_manager) {
            for (const auto &[id, client]: id_to_client) {
                RType::Packet::GameStart game_start(id);
                auto packet = package_manager->createPacket<RType::Packet::GameStart>(game_start, id);
                udp_handler->send(&packet, sizeof(packet), client.getIpAddress(), client.getPort());
            }
        }
        ...
        void spawnPlayers(std::shared_ptr<Network::UDPHandler> &udp_handler, std::shared_ptr<RType::Network::PackageManager> &package_manager, std::unique_ptr<ECS::Coordinator> &coordinator) {
            for (const auto &[id, client]: id_to_client) {
                ...
                RType::Packet::SpawnEntity entity_payload(player, name, 0, static_cast<float>(200 * id));
         ...
         */
        ECS::Entity getEntityFromPlayerID(PlayerID id) {
